...
const Hamburger = ({ setOpen, open }) => (
  ...
  >
    <span className="relative top-0 w-6 h-6 inline-flex flex-col justify-around ml-4 transition-all duration-200">
      ...
      <span className={`absolute -translate-y-1/2 w-full h-[2px] rounded bg-neutral-400 transition-all ${open ? 'top-1/2 -right-8 duration-700 opacity-0 transition-all' : 'top-[12px] right-0 group-hover:top-[12px] duration-200'}`}></span>
